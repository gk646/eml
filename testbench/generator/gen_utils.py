...
def generate_tensor_data(name, shape, data, dtype="qint32_t"):
...
def generate_tensor_empty(name, shape, size, dtype="qint32_t"):
    """Generate C++ tensor initialization and allocation."""
    shape_str = ', '.join(map(str, shape))
    string = ""
...
def generate_decl_tensor_range(name, shape, size, dtype="qint32_t"):
    """Generate C++ tensor initialization and allocation."""
    shape_str = ', '.join(map(str, shape))
    string = ""
    string += f"{generate_stack_array(name + "_data", size)}"
