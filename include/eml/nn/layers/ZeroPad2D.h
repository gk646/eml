...
template <typename T>
void ZeroPad2D::forward( const Tensor<T>& input, Tensor<T>& output )
{
    EML_ASSERT( output.h == input.h + padding.first * 2 && output.w == input.w + padding.second * 2, "Invalid shape" );

    int32_t channelOffOut = 0;
    int32_t channelOffIn = 0;
    for( int32_t c = 0; c < output.c; ++c )
    {
        int32_t matrixOffOut = channelOffOut;
        int32_t matrixOffIn = channelOffIn;
        ...
        channelOffOut += output.hw;
        channelOffIn += input.hw;
    }
}
