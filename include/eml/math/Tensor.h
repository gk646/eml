...
namespace eml
{
    ...
    struct Tensor final
    {
        ...
        // Sets all values to a random value between min and max (inclusive) - works for quantized tensors as well
...
namespace eml
{
    ...
    template <typename T>
    void Tensor<T>::random(const float min, const float max)
    {
        if constexpr (std::is_same_v<T, float>)
        {
            for (int i = 0; i < size; ++i)
            {
                data[i] = GetRandomFloat(min, max);
            }
        }
    }
